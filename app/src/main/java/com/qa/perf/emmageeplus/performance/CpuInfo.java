 ...
 *
 */
package com.qa.perf.emmageeplus.performance;
...
public class CpuInfo {
     ...
     * network traffic.
     *
     * @param totalBatt   the total batt
     * @param currentBatt the current batt
    ...
    public ArrayList<String> getCpuRatioInfo(String totalBatt, String currentBatt, String temperature, String voltage, String fps, boolean isRoot) {
        ...
        try {
            ...
            if (isInitialStatics) {
                ...
                StringBuilder totalCpuBuffer = new StringBuilder();
                if (null != totalCpu2 && totalCpu2.size() > 0) {
                    ...
                    for (int i = 0; i < (totalCpu.size() > totalCpu2.size() ? totalCpu2.size() : totalCpu.size()); i++) {
                        ...
                        totalCpuBuffer.append(cpuRatio).append(Constants.COMMA);
                    }
                } else {
                    processCpuRatio = "0";
                    totalCpuRatio.add("0");
                ...
                if (isPositive(processCpuRatio) && isPositive(totalCpuRatio.get(0))) {
                    ...
                    EmmageeService.bw.write(mDateTime2 + Constants.COMMA + ContextHelper.getTopActivity(context) + Constants.COMMA + heapData + pMemory
